...
// Windows-only cwd-integration helpers below. They are compiled on every
// platform so their logic is unit-tested on the macOS CI runner, but only
// `#[cfg(windows)]` code in session.rs calls them — hence dead-code on a
// non-Windows build. Allow that here rather than gate the definitions (which
// would break the mac-side tests).
